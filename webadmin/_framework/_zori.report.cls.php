...
//*************************
   public function addReport($Label, $File, $arrFilters=null, $Section="", $arrAdditional=null)
   {
      ...
      if(!file_exists("reports/$File.php")){
      ...
      if(count($this->Reports)>0){//
         ...
         foreach($this->Reports as $Section => $Reports){
            ...
            foreach($Reports->Report as $j => &$report){
               //print_rr($report);
               $report->ControlHTML = ZoriControl::renderControl("input", $report->html);

               $strReportControls .= "
                  <tr>
                     <td nowrap align='center'>". $report->ControlHTML ."</td>
                     <td nowrap align='left'><label for=\"". $report->html->id ."\" class='". $report->html->class ."'>". ZoriDetails::cleanColumnName($report->Label) ."</label></td>";
               $x = 0;
