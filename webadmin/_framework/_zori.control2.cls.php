...
//todo:
   ...
   {//print_rr($proto); //die("kldskok");
      ...
      switch($proto->Type)
      {
         ...
         case "file2":
            $control = new ZoriSimpleFileUploadControl($proto);
            break;
      ...
      if($this->sql != "")
      {
         if(strpos($this->sql,"GROUPING") === false)
         {
            if($this->Type == "datalist"){
               $this->sql = "SELECT '' AS ControlValue, ' - Select - ' AS ControlText UNION ALL ".$this->sql;
            }
...
//print_rr($this);
      if(is_array($arrOption)){
         ...
         {
            if($strGrouping != "" && substr($strGrouping, 0, strlen($strGrouping)) != " ") $innerHTML .= "<optGroup label=\"".strtoupper($strGrouping).".\">";
            //print_rr($Options);
            foreach($Options as $idx => $html)
            {
//if($this->ID == "lstClient"){print_rr($html);}
               ...
               if($this->Type == "datalist"){
                  ...
                  if($html->value == null || $html->value == "" || $html->value == 0 ){ $this->html->placeholder = $Options[0]->innerHTML;}
               }
               ...
               $attr = self::renderAttributes($html);
...
class ZoriSimpleFileUploadControl extends ZoriControl2
{
   public function __construct($proto){
      foreach($proto as $key => &$value){
         //vd($key); vd($value);
   ...
   public function render()
   {
      $this->html->id = $this->ID;
      $this->html->name = $this->Name;
      $this->html->class = "filestyle";
      $this->html->{"data-buttonBefore"} = "true";
      $this->html->{"data-placeholder"} = "No File";
      $this->html->{"data-size"} = "md";
         ...
         $this->HTML = "
           <div class='item_$this->ID'>
            <div class='input-group'>
               ...
               <span class='input-group-addon' data-toggle='popover' title='$this->CommentTitle' data-html='true'
               data-content=\"$this->Comment\" data-trigger='focus' id='info_$this->ID'><i class='glyphicon glyphicon-info-sign sm'></i></span>
            </div>
         </div>".$js;
      }
      else{
         $this->HTML = "<div class='item_$this->ID'><input type='file' $attr /></div>".$js;
      }
   }
}//endof ZoriSimpleFileUploadControl
?>